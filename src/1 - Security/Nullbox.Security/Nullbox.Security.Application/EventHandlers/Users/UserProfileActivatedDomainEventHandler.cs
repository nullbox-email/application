...
public class UserProfileActivatedDomainEventHandler : INotificationHandler<DomainEventNotification<UserProfileActivatedDomainEvent>>
{
    private readonly IMessageBus _messageBus;
    public UserProfileActivatedDomainEventHandler(IMessageBus messageBus)
    {
        _messageBus = messageBus;
    }

    public async Task Handle(
            DomainEventNotification<UserProfileActivatedDomainEvent> notification,
            CancellationToken cancellationToken)
    {
        var domainEvent = notification.DomainEvent;
        _messageBus.Publish(new UserProfileActivatedV1Event
        {
            Id = domainEvent.UserProfile.Id,
            Name = domainEvent.UserProfile.Name,
            EmailAddress = new UserProfileActivatedV1EmailAddressDto
            {
