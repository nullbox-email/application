...
public class DeliveryActionCompletedDomainEventHandler : INotificationHandler<DomainEventNotification<DeliveryActionCompletedDomainEvent>>
{
    private readonly IMessageBus _messageBus;

    public DeliveryActionCompletedDomainEventHandler(IMessageBus messageBus)
    {
        _messageBus = messageBus;
    }

    public async Task Handle(
            DomainEventNotification<DeliveryActionCompletedDomainEvent> notification,
            CancellationToken cancellationToken)
    {
        var domainEvent = notification.DomainEvent;
        _messageBus.Publish(new DeliveryActionCompletedV1Event
        {
