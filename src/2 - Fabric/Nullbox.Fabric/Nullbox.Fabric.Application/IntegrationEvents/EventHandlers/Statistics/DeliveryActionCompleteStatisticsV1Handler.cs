...
using Nullbox.Fabric.Application.Statistics.ProcessStatistics;
...
public class DeliveryActionCompleteStatisticsV1Handler : IIntegrationEventHandler<DeliveryActionCompleteStatisticsV1>
{
    private readonly ISender _mediator;

    public DeliveryActionCompleteStatisticsV1Handler(ISender mediator)
    {
        _mediator = mediator ?? throw new ArgumentNullException(nameof(mediator));
    }

    public async Task HandleAsync(
DeliveryActionCompleteStatisticsV1 message,
CancellationToken cancellationToken = default)
    {
        var command = new ProcessStatisticsCommand(
            id: message.Id,
            ...
            dropReason: message.DropReason is not null
                ? new ProcessStatisticsCommandDropReasonDto
                {
            ...
            quarantineReason: message.QuarantineReason is not null
                ? new ProcessStatisticsCommandQuarantineReasonDto
                {
