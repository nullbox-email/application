...
public class DeliveryActionCompleteProcessMailboxActivitiesV1Handler : IIntegrationEventHandler<DeliveryActionCompleteProcessMailboxActivitiesV1>
{
    private readonly ISender _mediator;

    public DeliveryActionCompleteProcessMailboxActivitiesV1Handler(ISender mediator)
    {
        _mediator = mediator ?? throw new ArgumentNullException(nameof(mediator));
    }

    public async Task HandleAsync(
            DeliveryActionCompleteProcessMailboxActivitiesV1 message,
            CancellationToken cancellationToken = default)
    {
        var command = new ProcessActivitiesCommand(
            id: message.Id,
            // [IntentIgnore]
            partitionKey: message.MailboxId ?? Guid.Empty,
