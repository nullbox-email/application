...
public class DeliveryActionCompleteProcessAliasActivitiesV1Handler : IIntegrationEventHandler<DeliveryActionCompleteProcessAliasActivitiesV1>
{
    private readonly ISender _mediator;

    public DeliveryActionCompleteProcessAliasActivitiesV1Handler(ISender mediator)
    {
        _mediator = mediator ?? throw new ArgumentNullException(nameof(mediator));
    }

    public async Task HandleAsync(
            DeliveryActionCompleteProcessAliasActivitiesV1 message,
            CancellationToken cancellationToken = default)
    {
        var command = new ProcessActivitiesCommand(
            id: message.Id,
            // [IntentIgnore]
            partitionKey: message.AliasId ?? Guid.Empty,
