...
using Nullbox.Fabric.Application.Statistics.ProcessRollups;
...
public class DeliveryActionCreateRollupsV1Handler : IIntegrationEventHandler<DeliveryActionCreateRollupsV1>
{
    private readonly ISender _mediator;

    public DeliveryActionCreateRollupsV1Handler(ISender mediator)
    {
        _mediator = mediator ?? throw new ArgumentNullException(nameof(mediator));
    }

    public async Task HandleAsync(DeliveryActionCreateRollupsV1 message, CancellationToken cancellationToken = default)
    {
        var command = new ProcessRollupsCommand(
            id: message.Id,
            ...
            dropReason: message.DropReason is not null
                ? new ProcessRollupsCommandDropReasonDto
                {
            ...
            quarantineReason: message.QuarantineReason is not null
                ? new ProcessRollupsCommandQuarantineReasonDto
                {
